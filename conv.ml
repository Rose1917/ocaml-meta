...
let _ = 
  ...
  let test_network = Nn.Net_marshal.load_net  "/home/march1917/recent/permanent/conv_network4" in
  (* train the model*)
  let train_set,_,train_label = Util.Mnist.load_train_data() in
  let step = 0.01 in
  let _cycles = Util.Mnist.train_epoc in
  for _it = 0000 to 60000 - 1 do
   ...
   Nn.Net_marshal.save_net test_network "/home/march1917/recent/permanent/conv_network5";


   let test_network = Nn.Net_marshal.load_net  "./permanent/conv_network5" in
